...
class cps(object):
    ...
    def get_contracts(self, session):
        ...
        if '?' in contracts_url:
            ...
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            contracts_url = contracts_url + account_key_url
    ...
    def create_enrollment(self, session, contractId, data, allowDuplicateCn=False):
        ...
        if '?' in create_enrollment_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            create_enrollment_url = create_enrollment_url + account_key_url
    ...
    def update_enrollment(self, session, enrollmentId, data):
        ...
        if '?' in update_enrollment_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            update_enrollment_url = update_enrollment_url + account_key_url
    ...
    def list_enrollments(self, session, contractId='optional'):
        ...
        if '?' in list_enrollments_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            list_enrollments_url = list_enrollments_url + account_key_url
    ...
    def get_enrollment(self, session, enrollmentId):
        ...
        if '?' in get_enrollment_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            get_enrollment_url = get_enrollment_url + account_key_url
    ...
    def get_change_status(self, session, enrollmentId, changeId):
        ...
        if '?' in get_change_status_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            get_change_status_url = get_change_status_url + account_key_url
    ...
    def get_change_history(self, session, enrollmentId):
        ...
        if '?' in get_change_history_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            get_change_history_url = get_change_history_url + account_key_url
    ...
    def cancel_change(self, session, enrollmentId, changeId):
        ...
        if '?' in cancel_change_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            cancel_change_url = cancel_change_url + account_key_url
    ...
    def delete_enrollment(self, session, enrollmentId):
        ...
        if '?' in delete_enrollment_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            delete_enrollment_url = delete_enrollment_url + account_key_url
    ...
    def get_certificate(self, session, enrollmentId, network='production'):
        ...
        if '?' in get_certificate_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            get_certificate_url = get_certificate_url + account_key_url
    ...
    def get_dv_change_info(self, session, endpoint):
        ...
        if '?' in dvChangeInfo_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            dvChangeInfo_url = dvChangeInfo_url + account_key_url
    ...
    def custom_post_call(self, session, headers, endpoint, data='optional'):
        ...
        if '?' in custom_url:
            ...
            #Replace & with ? if there is no query string in URL
            account_key_url = self.account_switch_key.translate(self.account_switch_key.maketrans('&','?'))
            custom_url = custom_url + account_key_url
